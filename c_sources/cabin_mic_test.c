...
int cabin_mic_setup(void){
    ...
    if (ret == -1) {
        ...
        printf("Settaggio configurazione microfoni CABIN completato.\n");
    }
    return ret;
}

/* All'interno di cabin_mic_test() */
int cabin_mic_test(void) {
    // Customizable paths and line offsets
    const char *input_chip_path = "/dev/gpiochip3";   // Path to GPIO chip for button
    unsigned int input_line = 21;                     // Line number for the button
    ...
    input_request = request_input_line(input_chip_path, input_line, "cabin-button");
    if (!input_request) {
    ...
    printf("Premere e tenere premuto il pulsante 'CABIN_PTT' fino al termine della registrazione.\n");
    fflush(stdout);
