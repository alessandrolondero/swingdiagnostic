...
int in2out2_mic_setup(void){
    /*settaggio microfoni*/
    printf("Settaggio microfoni...\n");
    int ret = system(
             "amixer set \"Capture\" on 79%; "
             "amixer -c 0 set \"INPGAR IN3R\" off; "
             "amixer -c 0 set \"MIXINR IN3R\" off; "
             "amixer -c 0 set \"INPGAL IN1L\" off;"
             "amixer -c 0 set \"INPGAR IN1R\" on; "
             );
    if (ret == -1) {
        ...
        printf("Settaggio configurazione microfoni IN2OUT completato.\n");
    }
    return ret;
}

/* All'interno di in2out2_mic_test() */
int in2out2_mic_test(void) {
    // Customizable paths and line offsets
    const char *input_chip_path = "/dev/gpiochip3";   // Path to GPIO chip for button
    unsigned int input_line = 26;                     // Line number for the button
    ...
    input_request = request_input_line(input_chip_path, input_line, "d-in-button");
    if (!input_request) {
    ...
    /* Set microphones */
    ret = in2out2_mic_setup();

    printf("Premere e tenere premuto il pulsante 'D_IN' fino al termine della registrazione.\n");
    fflush(stdout);
    ...
    while (!recording_done) {
        ...
        for (i = 0; i < ret; i++) {
            ...
            if (gpiod_edge_event_get_event_type(event) == GPIOD_EDGE_EVENT_FALLING_EDGE) {
                ...
                ret = system("amixer -c 0 set \"ADC L/R Swap\" on");
                if (ret == -1) {
                                fprintf(stderr, "Failed to execute amixer: %s\n", strerror(errno));
                            } else if (WEXITSTATUS(ret) != 0) {
                                fprintf(stderr, "amixer returned an error code: %d\n", WEXITSTATUS(ret));
                            } else {
                                printf("Successfully switched 'ON' ADC L/R Swap.\n");
                            }
                if (recorder_pid == -1) {
                    recorder_pid = fork();
                    if (recorder_pid == 0) {
                ...
                if (recorder_pid > 0) {
                    if (kill(recorder_pid, SIGTERM) == -1) {
                        ...
                        if (player_pid == 0) {
                            ...
                            printf("Fine riproduzione audio registrato.\nTest bench IN2OUT2 eseguito con successo.\n");

                            ret = system("amixer -c 0 set \"ADC L/R Swap\" off");
                            if (ret == -1) {
                                fprintf(stderr, "Failed to execute amixer: %s\n", strerror(errno));
                            } else if (WEXITSTATUS(ret) != 0) {
                                fprintf(stderr, "amixer returned an error code: %d\n", WEXITSTATUS(ret));
                            } else {
                                printf("Successfully switched 'OFF' ADC L/R Swap.\n");
                            }
